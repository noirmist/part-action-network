...
savePath = '/media/hci-jw/Plextor1tb/workspace/data/PartTestImages/';
for i=1:length(cat_dict)
    cat_name = cat_dict{i}{1};
    cat_num = cat_dict{i}{2};
    pics = dir(['/media/hci-jw/Plextor1tb/workspace/data/test_BBoxImage/' cat_name '_test']);
    pics = pics(3:end);
    cat_num = length(pics)
    
    ff=fopen([savePath cat_name '_test']);
    if(ff==-1)
        mkdir([savePath cat_name '_test']);
    end
    for m = 1:cat_num
        im_name = pics(m).name;
        disp(im_name);

        oriImg = imread(['/media/hci-jw/Plextor1tb/workspace/data/test_BBoxImage/' cat_name '_test/' im_name]);
        ...
        for j=1:length(parts)
            if(isfield(boxes,parts{j}))
                ...
                imwrite(patch, [savePath cat_name '_test/' im_name(1:end-4) '_' num2str(j) parts{j} '.jpg'], 'jpg');
                %id=id+1;
            else
